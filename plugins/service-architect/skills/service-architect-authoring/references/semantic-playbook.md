...
- Any number of `Sink` streams may reference that same endpoint, including multiple sinks in one service. Each sink remains an independent graph consumer while the transport endpoint and its endpoint function are shared.
- Within one service, at most one `Input` stream may reference an endpoint. This is the only endpoint-reference uniqueness restriction.
...
Before exporting, verify each edge is service-local and trace every cross-service path as `producer stream -> Sink -> connector endpoint -> Input -> consumer stream`. Treat a duplicated connector/endpoint declaration (not multiple references to one declared endpoint) or a type mismatch along that path as a modeling error.
