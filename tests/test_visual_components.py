...
def metadata():
    return {"version": 2, "groups": {"booking": {"name": "Booking", "fragments": [
        {"streams": [["validate", "check"]], "position": {"x": 10}},
        {"streams": [["reserve", "check"]]},
    ]}}}


class VisualComponentsTest(unittest.TestCase):
    def test_membership_and_optional_coordinates(self):
        self.assertEqual(metadata(), normalize_components(metadata(), [["validate", "check"], ["reserve", "check"]]))

    def test_legacy(self):
        self.assertEqual({"version": 2, "groups": {}}, normalize_components(None, []))

    def test_invalid_references_and_version(self):
        with self.assertRaisesRegex(ValueError, "stream"):
            normalize_components(metadata(), [["validate", "check"]])
        value = metadata()
        value["groups"]["booking"]["fragments"][1]["streams"] = [["reserve", "missing"]]
        with self.assertRaisesRegex(ValueError, "component"):
            normalize_components(value, [["validate", "check"], ["reserve", "check"]])
        with self.assertRaisesRegex(ValueError, "version"):
            normalize_components({**metadata(), "version": 99}, [])

    def test_invalid_positions_and_view_state(self):
        value = metadata()
        value["groups"]["booking"]["fragments"][0]["position"]["x"] = float("inf")
